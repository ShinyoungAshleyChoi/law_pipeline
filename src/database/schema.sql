...
CREATE TABLE IF NOT EXISTS law_list (
    ...
    total_cnt INT COMMENT '검색건수',
    page INT COMMENT '결과페이지번호',
    law_id INT COMMENT '법령ID',
    law_serial_no INT COMMENT '법령일련번호',
    ...
    promulgation_date INT COMMENT '공포일자 (YYYYMMDD)',
    promulgation_no INT COMMENT '공포번호',
    revision_type VARCHAR(100) COMMENT '제개정구분명',
    ministry_name VARCHAR(200) COMMENT '소관부처명',
    ministry_code INT COMMENT '소관부처코드',
    ...
    enforcement_date INT COMMENT '시행일자 (YYYYMMDD)',
...
CREATE TABLE IF NOT EXISTS law_content (
    ...
    chapter_section_no INT COMMENT '편장절관 일련번호',
    ministry_code INT COMMENT '소관부처코드',
    ministry_name VARCHAR(200) COMMENT '소관부처명',
    ...
    -- 조문 정보
    article_no INT COMMENT '조문번호',
    article_sub_no INT COMMENT '조문가지번호',
    article_yn VARCHAR(10) COMMENT '조문여부',
    article_title VARCHAR(1000) COMMENT '조문제목',
    article_enforcement_date INT COMMENT '조문시행일자',
    article_revision_type VARCHAR(100) COMMENT '조문제개정유형',
    article_move_before INT COMMENT '조문이동이전',
    article_move_after INT COMMENT '조문이동이후',
    article_change_yn VARCHAR(10) COMMENT '조문변경여부',
    article_content LONGTEXT COMMENT '조문내용',
    
    -- 항 정보
    paragraph_no INT COMMENT '항번호',
    ...
    -- 호 정보
    item_no INT COMMENT '호번호',
    item_content LONGTEXT COMMENT '호내용',
    
    -- 참고자료 및 부칙
    article_reference LONGTEXT COMMENT '조문참고자료',
    addendum_promulgation_date INT COMMENT '부칙공포일자',
    addendum_promulgation_no INT COMMENT '부칙공포번호',
    addendum_content LONGTEXT COMMENT '부칙내용',
    
    -- 별표 정보
    appendix_no INT COMMENT '별표번호',
    appendix_sub_no INT COMMENT '별표가지번호',
...
CREATE TABLE IF NOT EXISTS law_articles (
    ...
    title_change_yn VARCHAR(10) COMMENT '제명변경여부',
    korean_law_yn VARCHAR(10) COMMENT '한글법령여부',
    chapter_section_no INT COMMENT '편장절관',
    ministry_code INT COMMENT '소관부처 코드',
    ministry_name VARCHAR(200) COMMENT '소관부처명',
    phone_number VARCHAR(50) COMMENT '전화번호',
    enforcement_date INT COMMENT '시행일자 (YYYYMMDD)',
    revision_type VARCHAR(100) COMMENT '제개정구분명',
    ...
    article_move_before INT COMMENT '조문이동이전번호',
    article_move_after INT COMMENT '조문이동이후번호',
    article_change_yn VARCHAR(10) COMMENT '조문변경여부',
    article_content LONGTEXT COMMENT '조문내용',
...
CREATE TABLE IF NOT EXISTS batch_jobs (
    ...
    processed_laws INT DEFAULT 0 COMMENT '처리된 법령 수',
    processed_articles INT DEFAULT 0 COMMENT '처리된 조항 수',
    error_count INT DEFAULT 0 COMMENT '오류 발생 수',
...
CREATE TABLE IF NOT EXISTS sync_status (
    ...
    total_laws_count INT DEFAULT 0 COMMENT '총 법령 수',
    total_articles_count INT DEFAULT 0 COMMENT '총 조항 수',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
